...
function escape$3(html, encode3) {
  if (encode3) {
...
var Tokenizer_1 = class Tokenizer {
  ...
  emStrong(src2, maskedSrc, prevChar = "") {
    let match2 = this.rules.inline.emStrong.lDelim.exec(src2);
    if (!match2)
      return;
    ...
    if (!nextChar || nextChar && (prevChar === "" || this.rules.inline.punctuation.exec(prevChar))) {
      ...
      while ((match2 = endReg.exec(maskedSrc)) != null) {
        ...
        if (Math.min(lLength, rLength) % 2) {
          const text2 = src2.slice(1, lLength + match2.index + rLength);
          return {
            type: "em",
            raw: src2.slice(0, lLength + match2.index + rLength + 1),
            text: text2,
            tokens: this.lexer.inlineTokens(text2, [])
          };
        }
        const text = src2.slice(2, lLength + match2.index + rLength - 1);
        return {
          type: "strong",
          raw: src2.slice(0, lLength + match2.index + rLength + 1),
          text,
...
const { defaults: defaults$1$1 } = defaults$5.exports;
const {
  unescape: unescape$2
} = helpers;
...
function marked(src2, opt, callback) {
  ...
  try {
    const tokens = Lexer2.lex(src2, opt);
    if (opt.walkTokens) {
      marked.walkTokens(tokens, opt.walkTokens);
    }
...
marked.parseInline = function(src2, opt) {
  ...
  checkSanitizeDeprecation(opt);
  try {
    const tokens = Lexer2.lexInline(src2, opt);
    if (opt.walkTokens) {
      marked.walkTokens(tokens, opt.walkTokens);
    }
